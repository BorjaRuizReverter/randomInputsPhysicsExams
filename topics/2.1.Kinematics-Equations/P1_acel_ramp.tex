...
@
{\bf Kinematics - Equations.} A body initially at rest starts falling over a ramp with no friction with the ground. If after \Sexpr{(time)} seconds, the body have already moved \Sexpr{(distance)} m, what would be the module of its aceleration?
\begin{answers}{3}
    \bChoices[random]
    \Ans1 \label{resp2.1} \Sexpr{(result)}$\frac{m}{s^2}$\eAns  
    \Ans0 \Sexpr{(result_fake2)}$\frac{m}{s^2}$\eAns  
    \Ans0 \Sexpr{(result_fake3)}$\frac{m}{s^2}$\eAns
    \Ans0 \Sexpr{(result_fake4)}$\frac{m}{s^2}$\eAns 
    \Ans0 \Sexpr{(result_fake5)}$\frac{m}{s^2}$\eAns
    \eFreeze
...
Thefore, the right answer is the letter \textbf{(\REF*{resp2.1})}.
\end{solution}
\end{problem}